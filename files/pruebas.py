...
mi_lista_1_name = list(locals().keys())[list(locals().values()).index(mi_lista_1)];
...
print("Cadena con espacios a los extremos: " + cadena); # es lo mismo que trim() de JS
cadena_sin_espacios = cadena.strip();
print("Esta es una cadena sin espacios: " + "\"" + cadena_sin_espacios + "\"");

cadena_con_reemplazos = cadena.replace("es", "no es"); # replace() reemplaza un substring por otro
cadena_con_reemplazos = cadena.replace("cadena", "string");
print("Cadena con reemplazos: " + cadena_con_reemplazos);

array_de_strings = cadena.split(); # split() crea un array de strings a partir de una cadena
